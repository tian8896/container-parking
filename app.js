...
(function() {
  ...
  s1.onload = function() {
    ...
    s2.onload = function() {
      firebaseReady = true;
      // Load settings BEFORE login so user accounts are available
      initSettingsBeforeLogin();
      if (sessionStorage.getItem(LOGIN_KEY)) initApp();
    };
...
function initSettingsBeforeLogin() {
  ...
  try {
    ...
    ref.once('value', function(snap) {
      ...
      if (data.users) {
        ...
        if (fixed) ref.child('users').set(u);
    ...
    console.warn('initSettingsBeforeLogin error:', e);
  }
}
